...
const createChat = async function(type,chat){
...
const onMessage = async function(socket,io,msg){
    ...
    const socketIds = [];
    socketIds.push({username: msg.from,id: socket.id,otherUser: msg.to});
    if(msg.from !== msg.to){
        socketIds.push({username: msg.to,id: toSocketId,otherUser: msg.from});
    }
    await updateChat(socket,io,msg,socketIds);
    await updateMessage(socket,io,msg,socketIds);
};

module.exports = {verifyToken,onConnect,onDisconnect,onMessage};
